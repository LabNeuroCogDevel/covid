...
require(dplyr)
require(ggplot2)
require(tidyr)# treat data as global variable in functions
# this is a large (~180Mb) file
CONNDATA <- read.csv("~/Downloads/covidv3_conn_long.csv")# do only Y1
...
CONNDATA <- CONNDATA %>%
  ...
  filter(vrank < 2) %>%
  select(-vrank)
CONNDATA <- collapse_hemiside(CONNDATA)model <- NULL
require(neuroCombat) 
require(lme4)
require(dplyr)# treat data as global variable in functions
# this is a large (~180Mb) file
CONNDATA <- NULL# group by all the the columns we might use later
...
# }set_CONNDATA <- function() {
  ...
  conn_covid_resids <- lm(data=conn_covid, conn ~ fd_mean + study)
  conn_covid$resids <- residuals(conn_covid_resids)
  # updates global variable. large ~180Mb file
...
harmonize <- function(connName) {
  ...
  dat = matrix(conns, 1, length(conns))  gender <- as.factor(c(this$sex))
  sage <- c(this$age)
  mod <- model.matrix(~sage+gender)  data.harmonized <- neuroCombat(dat = dat, batch = batch, mod = mod, eb=FALSE)
  data.harmonized <- unlist(data.harmonized)
  # removes extra values from harmonization to match number of values
  data.harmonized3 <- unname(head(data.harmonized, length(this$age)))   this$data.harmonized3 <- data.harmonized3
  this$conn.original <- this$conn
  this$conn <- this$data.harmonized3  z <- LNCDR::zscore(this$conn)
...
default_model <- function(d) {
  if('srcHemi' %in% names(d))
    ...
    model <- conn ~ composite + age + sex.x + fd_mean + study.x
}model_conn <- function(this, model=NULL) {
  # default to ... the default model
  if(is.null(model)) model <- default_model(this)  # lmer if we have 'subj' in the forumla (indicating random effects)
  # lm if simple linear model
  with_rand_ef <- any(grepl('subj',as.character(model)))
  if(with_rand_ef)
    m <- lmer(data=this, model)
  else
    m <- lm(data=this, model)
...
plot_model_sexcolor <- function(m) {
  ...
  #  1st: x, 2nd: group, 3rd: facet  # get actual data to look like that
  mdata <- dplyr::rename(m@frame, predicted=conn, x=age, group=sex)  ggplot(pred) +
...
composite_issig <- function(m, p=.05) pval_composite(m) < p### START HERE
model_CONNDATA <-function(model=NULL){
  # updating global variable! only do it if it doesn't exist
  if(!exists("CONNDATA") || is.null(CONNDATA)) set_CONNDATA()  # if model is null, default set in model_conn is used
  #  model <- conn ~ age + sex + fd_mean + srcHemi + lat + (1|study/subj)
  all_conns <- unique(CONNDATA$connName)
  all_models <- lapply(all_conns, harmonize_and_model, model=default_model)
  names(all_models) <- all_conns  # remove models that failed
  # N.B. names(all_models) is no longer the same as all_conns
  all_models <- Filter(function(x) !is.null(x), all_models)  all_pvals <- sapply(all_models, pval_age)
  summary(all_pvals)
  # Min.     1st Qu.    Median      Mean   3rd Qu.      Max.
  #0.0000156 0.2011451 0.4555715 0.4596300 0.7302006 0.9766166  names(all_pvals[all_pvals <= .05])
  ...
  # "amOFC1_amOFC2"  "avmPFC_rACC"     # could use base::Filter (not dpylyr:filter) to retrict. but we already have
  # all the pavlues, so this is redudant 
  #sig_models <- Filter(function(m) age_issig(m, p=.01), all_models)  sig_models_05 <- all_models[all_pvals <= .05]
...
#  plot_model_sexcolor(sig_models_05[[1]])  return(model)
